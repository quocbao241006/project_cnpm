...
class Admin:
    ...
    def ui_view_and_update_order(self):
        try:
            ...
            if success:
                self.console.print(f"✅ CẬP NHẬT THÀNH CÔNG:[/bold green] Trạng thái đơn hàng {orderID} đã chuyển sang '{new_status}'.")
            else:
                self.console.print(f"❌ THẤT BẠI:[/bold red] Lỗi cập nhật trạng thái đơn hàng.")
        except Exception as e:
            self.console.print(f"❌ LỖI HỆ THỐNG: {e}")
